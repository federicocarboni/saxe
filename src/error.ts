const ERRORS = {
  ...
  INVALID_CHAR_REF: () => "Invalid char reference",
...
// Reasons to not subclass Error and use it instead of appending properties to Error:
// - ES6 classes extending builtins are difficult to replicate in ES5 (which is still a popular target).
// - `instanceof` is tempting but unreliable (across iframes and windows it never works, which would).
// - Using a code property on errors is a tried-and-true way to handle many different error conditions.

/** @internal */
export function makeError<T extends keyof ErrorCodes>(
  code: T,
